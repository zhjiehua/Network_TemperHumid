...
static void ENC28J60_SPI1_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
 	...
 	GPIO_SetBits(GPIOA, GPIO_Pin_4);						 //PD.2上拉
	
	//RST
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_3;				 // PB12 推挽   上拉
 	GPIO_Init(GPIOA, &GPIO_InitStructure);					 //根据设定参数初始化GPIOB.12
 	GPIO_SetBits(GPIOA, GPIO_Pin_3);						 //PB.12上拉

	//INT
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2;//PG6/7/8 推挽  上拉			 
 	GPIO_Init(GPIOA, &GPIO_InitStructure);					 //根据设定参数初始化//PG6/7/8
 	GPIO_SetBits(GPIOA,GPIO_Pin_2);//PG6/7/8上拉	
...
void ENC28J60_Reset(void)
{
	...
	SPI1_SetSpeed(SPI_BaudRatePrescaler_4);	//SPI2 SCK频率为36M/4=9Mhz
	
 	TIM2_Int_Init(100,719);//100Khz计数频率，计数到1000为10ms   72000000/720=100K   uip时钟
