...
#define   NET_REG_DATA			(*((volatile uint16_t *) (NET_BASE_ADDR + 4)))
...
#ifdef DM9000A_FLOW_CONTROL
	dm9k_phy_write(0x04, 0x01e1 | 0x0400);			/* 设置 自适应模式相容表 ,0x0400表示支持流控制*/
#else
...
void etherdev_init(void)
{
	...
	TIM6_Int_Init(10,719);//100Khz计数频率，计数到1000为10ms
}
